...
function updateHistogram(rootEl, data, proposedPrice, showTransition) {
  ...
  const x = d3.scaleLinear().domain(extent).range([left, right]);
  ...
  if (stdDev.empty()) {
    stdDev = svg
      ...
      .attr("class", "stddev");
    stdDev.append("rect").attr("class", "range-fill");
    stdDev.append("line").attr("class", "range-rule");
    const stdDevLabels = stdDev
      .append("g")
    ...
    stdDevLabels
      ...
      .attr("y1", -5)
      .attr("y2", 5);
    const stdDevLabelsText = stdDevLabels
      .append("text")
      .attr("text-anchor", d => d.anchor)
      .attr("dx", (d, i) => 8 * (i ? 1 : -1));

    stdDevLabelsText.append("tspan").attr("class", "stddev-text");
    stdDevLabelsText.append("tspan").attr("class", "stddev-text-label");
  }
  ...
  if (gBar.empty()) {
    gBar = svg.append("g").attr("class", "bars");
  }
  ...
  pp.select("line").attr("y1", ppOffset).attr("y2", bottom - top + 8);
  pp.select(".value").text(`$${proposedPriceStr} proposed`);
  ...
  const avgOffset = -55;
  if (avg.empty()) {
    avg = svg.append("g").attr("class", "avg");
  ...
  avg.select("line").attr("y1", avgOffset).attr("y2", bottom - top + 8);
  avg.select(".value").text(`${formatDollars(data.average)} average`);

  const bars = gBar.selectAll(".bar").data(bins);
  ...
  const t = showTransition ? d3.transition().duration(500) : svg;

  const stdDevWidth = x(stdDevMax) - x(stdDevMin);
  const stdDevTop = 85;
  stdDev = t.select(".stddev");
  stdDev.attr("transform", `translate(${[x(stdDevMin), stdDevTop]})`);

  stdDev
    .select("rect.range-fill")
    .attr("width", stdDevWidth)
    .attr("height", bottom - stdDevTop);

  stdDev.select("line.range-rule").attr("x2", stdDevWidth);

  stdDev
    .select(".chart-label.min .stddev-text")
    .text(formatDollars(stdDevMin))
    .attr("x", 0)
    .attr("dy", 0);

  stdDev
    .select(".chart-label.min .stddev-text-label")
    .text("-1 std dev")
    .attr("x", -8)
    .attr("dy", "15px");

  stdDev
    .select(".chart-label.max")
    .attr("transform", `translate(${[stdDevWidth, 0]})`);

  stdDev
    .select(".chart-label.max .stddev-text-label")
    .text("+1 std dev")
    .attr("x", 8)
    .attr("dy", "15px");

  stdDev.select(".chart-label.max .stddev-text").text(formatDollars(stdDevMax));
  ...
  const ticks = bins.map(d => d.min).concat([data.maximum]);

  const xa = d3.axisBottom().scale(x).tickValues(ticks).tickFormat((d, i) => {
  ...
  const ya = d3
    .axisLeft()
    .scale(d3.scaleLinear().domain(yd).range([bottom, top]))
    .tickValues(yd);
  ya.tickFormat(formatCommas);
  yAxis.call(ya).attr("transform", `translate(${[left - 2, 0]})`);
