...
describe("<ContractYear>", () => {
  it("renders correctly", () => {
    ...
    const curr = wrapper.find(
      `input[type="radio"][id="${props.idPrefix}current-year"]`
    );
    expect(curr.exists()).toBeTruthy();
    expect(curr.prop("value")).toBe("current");

    const oneYear = wrapper.find(
      `input[type="radio"][id="${props.idPrefix}one-year-out"]`
    );
    expect(oneYear.exists()).toBeTruthy();
    expect(oneYear.prop("value")).toBe("1");

    const twoYears = wrapper.find(
      `input[type="radio"][id="${props.idPrefix}two-years-out"]`
    );
