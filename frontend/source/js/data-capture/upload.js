...
const HAS_BROWSER_SUPPORT = supports.dragAndDrop() &&
  supports.formData() &&
...
export class UploadInput extends window.HTMLInputElement {
  ...
  attachedCallback() {
    ...
    if (this.hasAttribute("multiple")) {
      throw new Error(
        '<input is="upload-input"> does not currently ' +
  ...
  isFileValid(file) {
    ...
    return acceptsList.some(
      extOrType =>
        fileType === extOrType ||
        fileName.lastIndexOf(extOrType, fileName.length - extOrType.length) !==
          -1
    );
  }
}
...
export class UploadWidget extends window.HTMLElement {
  ...
  attachedCallback() {
    ...
    if ($input.length !== 1 || $input.attr("is") !== "upload-input") {
      throw new Error(
        "<upload-widget> must contain exactly one " +
    ...
    $("label", $el).after(
      '<span aria-hidden="true"> or drag and drop here.</span>'
    );
