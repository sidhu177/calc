...
(function uploadTests(QUnit, $) {
  ...
  function makeWidget(isDegraded, cb, options = {}) {
    ...
    if (options.fakeInitialFilename) {
      $("upload-widget", div).attr(
        "data-fake-initial-filename",
  ...
  advancedTest(
    "advanced upload sets fake filename if provided",
    assert => {
  ...
  advancedTest(
    "advanced upload does not allow non-accepted file types",
    assert => {
  ...
  advancedTest(
    'advanced upload allows any file when "accept" not specified',
    assert => {
