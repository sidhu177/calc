...
function makeFormHtml(extraOptions) {
  ...
  const $form = $("<div></div>").html(QUNIT_FIXTURE_DATA.AJAXFORM_TESTS_HTML);
  ...
  $form
    .find(`input[name="a_radio"][value="${options.aRadioSelectedValue}"]`)
    .attr("checked", true);

  const checkboxValues = Array.isArray(options.someCheckboxesValues)
    ? options.someCheckboxesValues
    : [options.someCheckboxesValues];
  ...
  checkboxValues.forEach(val => {
    $form
      .find(`input[name='some_checkboxes'][value=${val}]`)
      .attr("checked", true);
  });
...
formTest(
  "degraded form has truthy .isDegraded",
  {
    isDegraded: true
  },
  (assert, s) => {
...
formTest(
  "degraded form does not cancel form submission",
  {
    isDegraded: true
  },
  (assert, s) => {
...
advancedTest(
  "form_html replaces form & rebinds it",
  {
    fooValue: "hello"
  },
  (assert, s) => {
