...
QUnit.test("appendHighlightedTerm() works", assert => {
  assert.equal(
    appendHighlightedTerm($("<span></span>"), "superduperman", "super").html(),
    "<b>super</b>duperman"
  );

  assert.equal(
    appendHighlightedTerm($("<span></span>"), "superduperman", "uper").html(),
    "s<b>uper</b>d<b>uper</b>man"
  );

  // test to make sure it doesn't blow up on special-character-only search term
  assert.equal(
    appendHighlightedTerm($("<span></span>"), "superduperman", "{{{").html(),
    "superduperman"
  );
});
